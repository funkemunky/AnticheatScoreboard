...
public abstract class Anticheat implements Listener {
    ...
    public void addViolation(UUID uuid, String checkName, boolean cancelled, Predicate<Violation> toFilter, Consumer<Violation> toSet, String tags) {
        if(GeneralConfig.testMode && !AnticheatScoreboard.INSTANCE.anticheatManager.anticheatMap.get(Bukkit.getPlayer(uuid).getUniqueId()).equals(getAnticheatName())) return;
        ...
        if(GeneralConfig.testMode) {
            AnticheatScoreboard.INSTANCE.alerts.stream().filter(pl -> AnticheatScoreboard.INSTANCE.anticheatManager.anticheatMap.get(pl.getUniqueId()).equals(getAnticheatName())).forEach(pl -> pl.sendMessage(message));
        } else {
